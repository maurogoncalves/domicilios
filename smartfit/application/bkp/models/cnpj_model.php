...
function listarCnpjByIdEstCid($id,$estado,$cidade){
	if($id == 0){
		$sql = "select cc.*,c.id as id_cnpj_raiz from  cnpj cc  join cnpj_raiz c on cc.id_cnpj_raiz = c.id where cc.id_uf = ? and cc.id_municipio = ? ";
		$query = $this->db->query($sql, array($estado,$cidade));
	}else{
		$sql = "select cc.*,c.id as id_cnpj_raiz from  cnpj cc  join cnpj_raiz c on cc.id_cnpj_raiz = c.id where c.id = ? and cc.id_uf = ? and cc.id_municipio = ? ";
