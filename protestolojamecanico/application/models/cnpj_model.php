...
function listarBandeira(){	  
 ...
 function listarCnpj($id_contratante,$estado,$cidade,$cnpj){
   $sql1 = $sql2 = $sql3='';
	...
	if($cidade <> 0){
		$sql2 =" and cid.id = ? and cc.id_municipio = cid.id";
    }
	...
	$sql = "select c.cnpj_raiz,e.nome as uf ,cid.nome as cidade,cc.id,cc.cnpj,cc.nome,band.descricao_bandeira
			...
			left join estados e on cc.id_uf = e.id 
			left join cidades cid on cid.estado = e.id 
...
function listarCnpjByIdEstCid($id){
	...
	if($id == 0){
		...
		$sql = "select cc.*,c.id as id_cnpj_raiz  from  cnpj cc  join cnpj_raiz c on cc.id_cnpj_raiz = c.id  where c.id = ? ";
		$query = $this->db->query($sql, array($id));
	}
