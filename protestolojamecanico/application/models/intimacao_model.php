...
$sqlold = "select i.*,c.cnpj,cr.cnpj_raiz,DATE_FORMAT(i.data_ciencia,'%d/%m/%Y') as data_ciencia_br,DATE_FORMAT(i.prazo,'%d/%m/%Y') as prazo_br,ie.numero as num_ie,im.numero as num_im, n.descricao_natureza,
		aj.id AS id_aj,aj.data_previa_analise,aj.escritorio_parceiro,aj.status_juridico,
			...
			e.uf,descricao_bandeira,t.descricao_tributo ,tc.descricao_tributo_classificacao
				...
				join cnpj c on i.id_cnpj = c.id 
				join cnpj_raiz cr on c.id_cnpj_raiz = cr.id 
				join estados e on i.id_uf = e.id
				join cidades cid on cid.id = i.id_municipio 
				left join natureza n on n.id = i.id_natureza
		...
		aj.id AS id_aj,aj.data_previa_analise,aj.escritorio_parceiro,aj.status_juridico,
			...
			aj.campo_complementar_quatro AS quatro_aj,
			aj.campo_complementar_cinco AS cinco_aj,
			af.id id_af,af.tipo_intimacao,af.data_previa_atendimento,af.data_final,af.valores_defensaveis,af.valores_indefensaveis,af.status_fiscal,
						af.campo_complementar_um AS um_af,
			af.campo_complementar_dois AS dois_af,
			af.campo_complementar_tres AS tres_af,
			af.campo_complementar_quatro AS quatro_af,
			af.campo_complementar_cinco AS cinco_af	,DATE_FORMAT(i.periodo_auditado_inicial,'%d/%m/%Y') as periodo_auditado_inicial_br,	DATE_FORMAT(i.periodo_auditado_final,'%d/%m/%Y') as periodo_auditado_final_br,
			DATE_FORMAT(af.data_previa_atendimento,'%d/%m/%Y') as data_previa_atendimento_br,	DATE_FORMAT(af.data_final,'%d/%m/%Y') as data_final_br,	DATE_FORMAT(aj.data_previa_analise,'%d/%m/%Y') as data_previa_analise_br,
			e.uf,descricao_bandeira,t.descricao_tributo ,tc.descricao_tributo_classificacao,tc.descricao_tributo_classificacao as id_classificacao
				from intimacoes i 
				left join cnpj c on i.id_cnpj = c.id 
				left join cnpj_raiz cr on c.id_cnpj_raiz = cr.id 
				left join estados e on i.id_uf = e.id
	...
	public function listarInfracaoById($id){
		$sql = "select i.*,c.cnpj,cr.cnpj_raiz,DATE_FORMAT(i.data_ciencia,'%d/%m/%Y') as data_ciencia_br,
		DATE_FORMAT(i.prazo,'%d/%m/%Y') as prazo_br,ie.numero as num_ie,im.numero as num_im,e.id as id_uf,
		c.id_municipio,cr.id as id_cnpj_raiz,c.ID as id_cnpj,arq.arquivo as arq,
			aj.id AS id_aj,aj.data_previa_analise,aj.escritorio_parceiro,aj.status_juridico,
			aj.campo_complementar_um AS um_aj,
			aj.campo_complementar_dois AS dois_aj,
			aj.campo_complementar_tres AS tres_aj,
 ...
 function listarEsfera(){
   ...
   else
   {
	...
	public function contaIntimacaoesByUf($uf){
		...
		if($uf <> 0){
			$sql = "select  count(*) as total from intimacoes i join cnpj c on i.id_cnpj = c.id left join estados e on i.id_uf = e.id  where i.status = 1"; 
			$query = $this->db->query($sql, array());
		}else{
			$sql = "select  count(*) as total from intimacoes i join cnpj c on i.id_cnpj = c.id  left join estados e on i.id_uf = e.id  where e.uf = ? and i.status = 1"; 
