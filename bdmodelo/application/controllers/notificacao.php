...
class Notificacao extends CI_Controller {
 ...
 function cadastrar_notificacao(){
	...
	<BR>
	<a href="https://bdwebgestora.com.br/domicilios/smartfit/index.php/notificacao/tracking?id='.$id.'" target="_blank" >https://bdwebgestora.com.br/domicilios/smartfit </a>
	<BR><BR>
...
function alterar_notificacao(){
	...
	function arquivosApp(){	
		...
		for($i=0;$i<=4;$i++){
			...
			if (file_exists($filename)) {
				$urlArr[$i]['url'] = 'https://bdwebgestora.com.br/domicilios/smartfit/arquivos/notificacoes/'.$id.'-'.$i.'.pdf';				
			}else{
				$urlArr[$i]['url'] = '-';
			}
		}	
 ...
 function listarNaturezaJson(){
	...
	@$this->email->subject('Sistema BD Webdomicilios - Envio de Encaminhamento de Notificação' );
		...
		$b = getcwd();
		$caminho = $b.'/arquivos/enc_notificacoes/'.$dadosInfracao[0]->arq;
