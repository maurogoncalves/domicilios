...
class Infracoes extends CI_Controller {
 ...
 function cadastrar_infracao(){
	...
	$periodo_auditado_inicial = str_replace("/","-",$periodo_auditado_inicial);
	$periodo_auditado_final = str_replace("/","-",$periodo_auditado_final);
	
	if(!empty($periodo_auditado_inicial)){
		$periodo_auditado_inicial_arr = explode('/',$periodo_auditado_inicial);
		$periodo_auditado_inicial_intimacao = $periodo_auditado_inicial_arr[2].'-'.$periodo_auditado_inicial_arr[1].'-'.$periodo_auditado_inicial_arr[0];
	}else{		
		$periodo_auditado_inicial_intimacao = '1900-01-01';
	}	
	
	if(!empty($periodo_auditado_final)){
		$periodo_auditado_final_arr = explode('/',$periodo_auditado_final);
		$periodo_auditado_final_intimacao = $periodo_auditado_final_arr[2].'-'.$periodo_auditado_final_arr[1].'-'.$periodo_auditado_final_arr[0];
	}else{		
		$periodo_auditado_final_intimacao = '1900-01-01';
	}	
	...
	$dados = array(
		...
		'numero_os' => $numero_os,
		'periodo_auditado_inicial' =>  strtotime($periodo_auditado_inicial),
		'periodo_auditado_final' =>  strtotime($periodo_auditado_final)
		);		
		...
		'responsavel_fiscalizacao' => $responsavel_fiscalizacao,
		'numero_os' => $numero_os,
		'periodo_auditado_inicial' =>  $periodo_auditado_inicial_intimacao,
		'periodo_auditado_final' =>  $periodo_auditado_final_intimacao
		);		
	...
	foreach($emails as $valor){
		...
		<BR>
		<a href="https://bdwebgestora.com.br/domicilios/smartfit//index.php/infracoes/tracking?id='.$id.'" target="_blank" >https://bdwebgestora.com.br/domicilios/smartfit/ </a>
		<BR><BR>
...
function encaminhar(){
	...
	@$this->email->subject('Sistema BD Webdomicilios - Envio de Encaminhamento de Infração' );
		...
		Entre no sistema e clique nesse link abaixo para acessa o tracking.
		<BR>
		<a href="https://bdwebgestora.com.br/domicilios/smartfit/index.php/infracoes/tracking?id='.$idInfracao.'" target="_blank" >https://bdwebgestora.com.br/domicilios/smartfit/ </a>
		<BR><BR>
	...
	function arquivosApp(){	
		...
		for($i=0;$i<=4;$i++){
			...
			if (file_exists($filename)) {
				$urlArr[$i]['url'] = 'https://bdwebgestora.com.br/domicilios/smartfit/arquivos/infracoes/'.$id.'-'.$i.'.pdf';				
			}else{
				$urlArr[$i]['url'] = '-';
			}
		}	
